...
Table \ref{tab:syntResults} summarize the obtained results and reports the best selected model for each scenario. \tcolor{blue}{The Time column highlights how the most expensive task is the generation of the ${compatibles}$ set. This is indeed expected, as it requires to check all constraints in $D[A]$ against all traces in $L^+$.}
