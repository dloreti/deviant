...
& & & \textit{\sheriff}:  \fpeval{109.74 + 3.32}s  & \emph{assess\_eligibility)} & \emph{assess\_eligibility)} \\
...
& & & \textit{\sheriff}:  \fpeval{94.51 + 2.96}s  & \emph{receive\_negative\_ feedback)} & \emph{receive\_negative\_feedback)}\\
& & & Optimisation: 1.379s & & \\
\bottomrule
\end{tabular}
}
