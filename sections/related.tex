...
Interestingly, \cite{slaats_weighing_2021} also reports a demonstration of the reasons why Declare is not able to perfectly separate any pair of positive/negative traces. Consistently with this finding, our approach aims at discovering a model that accepts all traces in $L+$ and rejects as many traces in $L^-$ as possible. Despite its limits, we believe that Declare's diffusion in both industry and academia motivates the need to further investigate process discovery with this notation.}
