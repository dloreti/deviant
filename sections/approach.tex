...
\end{lstlisting}
using a specific predicate (\lstinline[language=prolog]$activity/1$) representing the set of all activities.\footnote{The actual ASP code leverages \textsc{Clingo} function symbols and the fact that a constraint can be derived from the selected ones is represented by means of the predicate \lstinline[language=prolog]$holds/1$: e.g., \lstinline[language=prolog]$holds(precedence(A,B))$.}
...
\end{lstlisting}
%
where \lstinline[language=prolog]$choice/2$ is the input to the ASP program, encoding the above \textit{\sheriff} function. The 
